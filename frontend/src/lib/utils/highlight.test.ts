...
describe('highlightText', () => {
  ...
  it('wraps a case-insensitive match in <mark>', () => {
    expect(highlightText('Find the Fish', 'fish')).toBe(
      'Find the <mark>Fish</mark>'
    );
  });

  it('wraps every occurrence', () => {
    expect(highlightText('ab ab AB', 'ab')).toBe(
      '<mark>ab</mark> <mark>ab</mark> <mark>AB</mark>'
    );
  });
  ...
  it('treats regex metacharacters in the query as literals', () => {
    expect(highlightText('match a.b.c here', 'a.b.c')).toBe(
      'match <mark>a.b.c</mark> here'
    );
    // Without escaping this would match anything-anything-anything.
    expect(highlightText('match abc here', 'a.b.c')).toBe(
      'match abc here'
    );
  });
