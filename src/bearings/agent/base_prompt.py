...
BASE_PROMPT = (
    ...
    "\n"
    "For shell commands, use `mcp__bearings__bash` — same interface "
    "as the built-in Bash tool, but Bearings streams its output to "
    "the terminal pane line-by-line as the command runs instead of "
    "dumping one final block on exit. The built-in `Bash` is "
